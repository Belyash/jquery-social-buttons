(function ($, window, document, undefined) {
        ...
        init: function (options, el) {
            ...
            if (oSocial) {
                ...
                */
                ...
                if (oSocial.url) {
                    $.getScript(
                        oSocial.url + self.shareUrl + "&callback=jQuery.fn.socialShares." + network + "SetCount",
                ...
                if (oSocial.share) {
                    $el.on("click.socialShares", function () {
                        window.open(
    ...
    $.fn.socialShares = function(options) {
        return this.each(function () {
            var socialShares = Object.create(SocialShares);

            if (SocialShares[options]) {
                return SocialShares[options].apply(this, Array.prototype.slice.call(arguments, 1));
            } else if (typeof options === 'object' || typeof options === 'undefined') {
                return socialShares.init(options, this);
            } else {
    ...
    for (var network in Socials) {
        if (Socials.hasOwnProperty(network)) {
            $.fn.socialShares[network + "SetCount"] = Socials[network].callback.bind(Socials[network]);
        }
    }
